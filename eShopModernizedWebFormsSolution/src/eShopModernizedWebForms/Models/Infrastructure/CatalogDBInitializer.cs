...
namespace eShopModernizedWebForms.Models.Infrastructure
{
    public class CatalogDBInitializer
    {
        ...
        private void AddCatalogTypes(CatalogDBContext context)
        {
            ...
            int seqId = GetSeqFromDB(context, DBCatalogSequenceName);
        ...
        private void AddCatalogBrands(CatalogDBContext context)
        {
            ...
            int seqId = GetSeqFromDB(context, DBBrandSequenceName);
        ...
        private void AddCatalogItems(CatalogDBContext context)
        {
            ...
            foreach (var item in items) { item.Id = indexGenerator.GetNextSequenceValue(context); context.CatalogItems.Add(item); }
            context.SaveChanges();
        }
        ...
        private IEnumerable<CatalogItem> GetCatalogItemsFromFile(CatalogDBContext context)
        {
            ...
            string[] req = { "catalogtypename", "catalogbrandname", "description", "name", "price", "pictureFileName" };
            string[] opt = { "availablestock", "restockthreshold", "maxstockthreshold", "onreorder" };
            var headers = GetHeaders(path, req, opt);
            var typeMap = context.CatalogTypes.ToDictionary(ct => ct.Type, ct => ct.Id);
            var brandMap = context.CatalogBrands.ToDictionary(ct => ct.Brand, ct => ct.Id);
            return File.ReadAllLines(path).Skip(1)
                .Select(row => Regex.Split(row, ",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)"))
                .Select(cols => BuildItem(cols, headers, typeMap, brandMap)).Where(x => x != null);
        }
        ...
        private static CatalogItem BuildItem(string[] cols, string[] headers, Dictionary<string, int> typeMap, Dictionary<string, int> brandMap)
        {
            ...
            if (!decimal.TryParse(cols[Array.IndexOf(headers, "price")].Trim('"').Trim(), NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture, out decimal price)) return null;
        ...
        private static int GetSeqFromDB(CatalogDBContext context, string seqName)
        {
