...
namespace eShopLegacyWebForms.Models.Infrastructure
{
    public class CatalogDBInitializer
    {
        ...
        public void Seed(CatalogDBContext context)
        {
            ...
            {
                logger.LogWarning(ex, "HiLo sequence scripts failed (may already exist). Continuing.");
            }
        ...
        {
            return GetCatalogBrandsFromFile(hostingEnvironment.ContentRootPath);
        }
            ...
            Dictionary<string, int> catalogTypeIdLookup, Dictionary<string, int> catalogBrandIdLookup)
        {
            ...
            if (availableStockIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(availableStockString) && int.TryParse(availableStockString, out int availableStock))
                {
            ...
            if (restockThresholdIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(restockThresholdString) && int.TryParse(restockThresholdString, out int restockThreshold))
                {
            ...
            if (maxStockThresholdIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(maxStockThresholdString) && int.TryParse(maxStockThresholdString, out int maxStockThreshold))
                {
            ...
            if (onReorderIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(onReorderString) && bool.TryParse(onReorderString, out bool onReorder))
                {
        ...
        {
            // dBSequenceName is an internal constant — not user input. Suppress the SQL injection warning.
...
#pragma warning restore EF1002
        ...
        {
            var scriptFilePath = Path.Combine(hostingEnvironment.ContentRootPath, scriptFile);
