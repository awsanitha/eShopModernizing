...
{
    /// <summary>
    /// Initializes the catalog database with seed data.
    /// Replaces the EF6 CreateDatabaseIfNotExists initializer pattern with explicit EF Core migration.
    /// </summary>
    public class CatalogDBInitializer
    {
        ...
        private static IEnumerable<CatalogBrand> GetCatalogBrandsFromFile()
        {
            var contentRootPath = AppDomain.CurrentDomain.BaseDirectory;
        ...
        {
            // Use a pre-validated sequence name to prevent SQL injection.
            // The sequence name comes from constants defined in this class, not user input.
            if (!System.Text.RegularExpressions.Regex.IsMatch(dBSequenceName, @"^[a-zA-Z_][a-zA-Z0-9_]*$"))
            {
                throw new ArgumentException($"Invalid sequence name: {dBSequenceName}");
            }
#pragma warning disable EF1002 // Risk of vulnerability to SQL injection - sequence name is validated above
