...
namespace eShopLegacyMVC.Models.Infrastructure
{
    public class CatalogDBInitializer
    {
        ...
        {
            // Use allowed sequence names only to prevent SQL injection
            string safeName = dBSequenceName switch
            {
                DBCatalogSequenceName or DBBrandSequenceName => dBSequenceName,
                _ => throw new ArgumentException($"Unknown sequence name: {dBSequenceName}")
            };
#pragma warning disable EF1002 // Sequence names are validated above
            var result = context.Database.SqlQueryRaw<long>($"SELECT NEXT VALUE FOR {safeName}").ToList();
