...
namespace eShopLegacyMVC.Models.Infrastructure
{
    public class CatalogDBInitializer
    {
        ...
        private void AddCatalogItemPictures()
        {
            ...
            var picPath = new DirectoryInfo(Path.Combine(contentRootPath, "Pics"));
            foreach (var f in picPath.GetFiles()) f.Delete();
            ZipFile.ExtractToDirectory(Path.Combine(contentRootPath, "Setup", "CatalogItems.zip"), picPath.ToString());
        }
    }
}
