...
namespace eShopModernizedMVC.Models.Infrastructure
{
    public class CatalogDBInitializer
    {
        ...
        public CatalogDBInitializer(CatalogItemHiLoGenerator indexGenerator, string contentRootPath)
        {
            this.indexGenerator = indexGenerator;
            this.useCustomizationData = CatalogConfiguration.UseCustomizationData;
        ...
        private IEnumerable<CatalogType> GetCatalogTypesFromFile()
        {
            ...
            return File.ReadAllLines(csvFileCatalogTypes).Skip(1).Select(x => CreateCatalogType(x)).Where(x => x != null);
        }

        static CatalogType CreateCatalogType(string type)
        {
            type = type.Trim('"').Trim();
            if (string.IsNullOrEmpty(type)) throw new Exception("catalog Type Name is empty");
        ...
        static IEnumerable<CatalogBrand> GetCatalogBrandsFromFile()
        {
            ...
            return File.ReadAllLines(csvFileCatalogBrands).Skip(1).Select(x => CreateCatalogBrand(x)).Where(x => x != null);
        }

        static CatalogBrand CreateCatalogBrand(string brand)
        {
            brand = brand.Trim('"').Trim();
            if (string.IsNullOrEmpty(brand)) throw new Exception("catalog Brand Name is empty");
        ...
        static CatalogItem CreateCatalogItem(string[] column, string[] headers, Dictionary<string, int> catalogTypeIdLookup, Dictionary<string, int> catalogBrandIdLookup)
        {
            ...
            {
                string value = column[availableStockIndex].Trim('"').Trim();
                if (!string.IsNullOrEmpty(value) && int.TryParse(value, out int availableStock))
            ...
            {
                string value = column[restockThresholdIndex].Trim('"').Trim();
                if (!string.IsNullOrEmpty(value) && int.TryParse(value, out int restockThreshold))
            ...
            {
                string value = column[maxStockThresholdIndex].Trim('"').Trim();
                if (!string.IsNullOrEmpty(value) && int.TryParse(value, out int maxStockThreshold))
            ...
            if (onReorderIndex != -1)
            {
                string value = column[onReorderIndex].Trim('"').Trim();
                if (!string.IsNullOrEmpty(value) && bool.TryParse(value, out bool onReorder))
        ...
        {
            if (!useCustomizationData) return;
