...
if (!string.IsNullOrEmpty(appInsightsKey))
{
    ...
    builder.Services.AddSingleton<Microsoft.ApplicationInsights.Extensibility.ITelemetryInitializer, eShopModernizedMVC.MyTelemetryInitializer>();
}
...
if (!useMockData)
{
    var connectionString = builder.Configuration.GetConnectionString("CatalogDBContext")
        ?? "Server=(localdb)\\mssqllocaldb;Database=eShopModernizedMVCDB;Trusted_Connection=True;MultipleActiveResultSets=true";
...
if (!useMockData)
{
    ...
    try
    {
        ...
        var initializer = new CatalogDBInitializer(
            scope.ServiceProvider.GetRequiredService<eShopModernizedMVC.Models.CatalogItemHiLoGenerator>(),
            env);
